...
def line_loading_histogram(
    ...
    *,
    ...
    if POLARS_AVAILABLE:
        ...
        # Convert to plain lists first: passing pandas Index/ndarray directly
        # makes polars route through its pandas interop path, which raises
        # ImportError when pyarrow isn't installed (it isn't a project
        # dependency here).
        df = pl.DataFrame(
            {
                "line": max_loading.index.tolist(),
                "loading_pct": max_loading.to_numpy().tolist(),
            }
        )
...
ANALYSIS_TYPES: dict[str, Callable[..., dict[str, Any]]] = {
