...
export const loadingNetworks = writable(false);
...
export const selectedNetworks = derived(
	...
	([$networksList, $selectedNetworkIds]) => {
		return $networksList.filter((network) => $selectedNetworkIds.has(network.id));
	}
);

export const hasSelection = derived(selectedNetworkIds, ($ids) => $ids.size > 0);
